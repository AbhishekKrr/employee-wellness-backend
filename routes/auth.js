...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const newUser = await pool.query(
      `INSERT INTO users 
      (name, email, password, role, unit, designation, department, age, phone, ailments, height, weight, blood_group, date_of_birth) 
...
router.get('/me', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT name FROM users WHERE id = $1', [req.user.id]);
